...
export function CanvaReportDeck() {
  ...
  const isFinalSlide = current.layout === "thankYou" || current.layout === "qa"
  ...
  const body = (() => {
    if (current.layout === "thankYou") return <div className={styles.finalLayout}><div className={styles.finalCopy}><h1 className={styles.finalTitle}><em>Thank</em><strong>You</strong></h1><p className={styles.finalLead}>{current.lead}</p><div className={styles.appreciation}><span>Special thanks to</span><p>LOCUS-T and DigitalBee<br />My industrial supervisor<br />My academic supervisor<br />The team and fellow interns</p></div></div><article className={styles.memePlaceholder} aria-label="Humorous presentation-ending meme thanking the audience for their attention" role="img"><b>Thank You meme</b><span>Image required: /public/images/thank-you-meme.jpg</span></article><p className={styles.memeCaption}>Finally made it.</p><p className={styles.finalTakeaway}>This internship started with <strong>curiosity</strong> and ended with a clearer direction for my future.</p></div>
    if (current.layout === "qa") return <div className={[styles.finalLayout, styles.qaLayout].join(" ")}><div className={styles.finalCopy}><h1 className={styles.finalTitle}><strong>Questions</strong><em>&amp; Answers</em></h1><p className={styles.finalLead}>{current.lead}</p><p className={styles.presenterLine}>Nur Iman Binti Noor Azizi<span>Developer Intern</span></p></div><article className={styles.memePlaceholder} aria-label="Humorous Q and A meme encouraging the audience to ask the presenter before ChatGPT" role="img"><b>Q&amp;A meme</b><span>Image required: /public/images/qa-chatgpt-meme.jpg</span></article><p className={styles.memeCaption}>ChatGPT later. <strong>Ask me first.</strong></p></div>
