...
import { Bot, Cloud, FileText, MessageCircle, Play, Sparkles, Workflow } from "lucide-react"
...
export function CanvaReportDeck() {
  ...
  const body = (() => {
    ...
    if (current.layout === "timeline") {
      ...
      return <div className={styles.timelineLayout}>{title}<p className={styles.timelineSubtitle}>{current.subtitle}</p><div className={styles.timeline}>{phases.map((phase, index) => { const Icon = phase.icon; const SupportingIcon = phase.supportingIcon; return <article key={phase.title} className={index === 2 ? styles.timelineFocus : ""}><div className={styles.phaseHeader}><b>{String(index + 1).padStart(2, "0")}</b><span className={styles.phaseIcon} aria-label={phase.label}><Icon aria-hidden="true" size={18} />{SupportingIcon ? <SupportingIcon aria-hidden="true" size={13} /> : null}</span></div><strong>{phase.title}</strong><p>{phase.description}</p><small><span>Output</span> · {phase.output}</small>{phase.note ? <em>{phase.note}</em> : null}</article> })}</div><button type="button" className={styles.trailer} onClick={() => setActive((index) => Math.min(index + 1, CHATBOT_SLIDES.length - 1))} aria-label="Play project preview and go to the next slide"><Play aria-hidden="true" fill="currentColor" size={14} /><span><b>Play project preview</b><small>35–45 second project montage</small></span></button></div>
    }
