...
export function CanvaReportDeck() {
  ...
  const body = (() => {
    if (current.layout === "finalQna") return <div className={styles.finalLayout}><div className={styles.finalCopy}><h1 className={styles.finalTitle}><em>Thank You</em><strong>&amp; Q&amp;A</strong></h1><p className={styles.finalLead}>{current.lead}</p><p className={styles.finalQuestion}>{current.subtitle}</p><div className={styles.appreciation}><span>Special thanks to</span><p>LOCUS-T and DigitalBee<br />My industrial supervisor<br />My academic supervisor<br />The team and fellow interns</p></div></div><div className={styles.memeStack}><article className={[styles.memePlaceholder, styles.doneMeme].join(" ")} aria-label="Humorous presentation-ending meme thanking the audience for their attention" role="img"><b>Presentation-done meme</b><span>Image required: /public/images/presentation-done-meme.jpg</span></article><article className={[styles.memePlaceholder, styles.qaMeme].join(" ")} aria-label="Humorous question-and-answer meme suggesting ChatGPT" role="img"><b>Q&amp;A meme</b><span>Image required: /public/images/qa-chatgpt-meme.jpg</span></article><p className={styles.memeCaption}>ChatGPT later. <strong>Ask me first.</strong></p></div><p className={styles.finalTakeaway}>This internship started with <strong>curiosity</strong> and ended with a clearer direction for my future.</p></div>
